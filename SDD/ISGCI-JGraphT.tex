...
Um einerseits die Darstellung der Informationen durch \G{JGraphT} zu erhalten, werden weiterhin eine Repräsentation des Graphen im \G{(Simple)DirectedGraph} verwaltet. Außerdem werden die Nodeviews und Edgeviews durch das entsprechende Pendant in \G{JGraphX}, also Knoten und Kanten, ersetzt. Damit hat man also den asymptotisch gleichen Aufwand der Datenhaltung. Diese Knoten und Kanten in der \G{JGraphX}-Darstellung werden nun als Cells (\G{\texttt{extends Object}}) verwaltet. Sie sind somit weiterhin im \G{DirectedGraph} erreichbar. Bei der Implementierung ist besonders darauf zu achten, dass der Zugriff auf Kanten und Knoten des Graphen effizient umgesetzt wird, damit \G{Userexperience} und \G{Usability} nicht unter langen Wartezeiten leidet.\\\\
Die Eigenschaften des ISGCIGraphs werden so gesetzt, dass sie denen des vorigen \G{SimpleDirectedGraphs} entsprechen, sodass gilt: "`protected boolean multigraph = false"' und "`allowLoops = false;"' (gerichtet, keine Kreise und keine Multikanten) haben. Diese Funktionen beinhalten unter anderem: 
{\small
\begin{itemize}
...
\item Export(.svg) des Graphen verläuft über JGraphX, die grundlegende Funktionalität zur Verfügung stellt
\item Zeichnen des Graphen auf dem von JGraphX mitgelieferten Canvas\\
\end{itemize}
}
\ \\
Dazu müssen folgende Klassen angepasst werden:
{\small 
\begin{description}
...
Die bereits existierende Klasse \G{ISGCIGraphCanvas.java} aus dem ISGCI Projekt wird so verändert, dass sie die Funktionalität der \G{mxGraphics2Dcanvas.java} realisiert. Das Zeichnen des Graphen wird so erreicht, dass die Walker, die mit JGraphT über einen SimpleDirectedGraph laufen die notwendigen Knoten und Kanten auslesen und dann mit den entsprechenden Eigenschaften dem zu zeichnenden Graphen hinzufügen. Darauf wird anschließend das Layout berechnet und gezeichnet. Die Klassen \G{NodeView.java}  und \G{EdgeView.java} finden somit keine Verwendung mehr für das Layout oder zum zeichnen und können bedenkenlos entfernt werden, nachdem alle Verbindungen ersetzt wurden. 
...
\begin{itemize}
\item Menu-Bar-Parent \G{View}; \G{Sidebar visible}, welche in der \G{ISGCIMainFrame} Klasse implementiert wird(Checkmark). 
\item Kontext-Menü; \G{Show sidebar}, vgl. Kontextmenü. 
\item Menu-Bar-Parent \G{Selected}, \G{Show sidebar}, welche in der \G{ISGCIMainFrame} Klasse implementiert wird. 
\end{itemize} 
Es existiert ein Button, um die Sidebar ein-/ auszublenden, falls sie geschlossen/geöffnet ist. Das ist zudem auch über das Menu-Bar-Parent \G{View}, \G{Sidebar visible} möglich.\\
Um die Graphclasses.org-Homepage zu parsen wird die Java-Bibliothek \G{Jsoup} benutzt. In der Klasse \G{Sidebar} werden die entsprechenden vorgeparsten Informationen von der Homepage in eine passende HTML-Form gebracht. 
...
Wird wie bisher in der ISGCIGraphCanvas.java Klasse nur jetzt über Standardfunktionalitäten aus der Bibliothek JGraphX realisiert.
